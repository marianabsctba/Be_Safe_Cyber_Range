window.BE_SAFE_LABS = [
  ...
  {id:"patient-zero",order:2,title:"Paciente Zero",short:"Reconstrua a timeline e encontre o primeiro endpoint comprometido.",track:"DFIR",level:"JR",duration:"35–50 MIN",status:"planned",accent:"#3de7ff",skills:["WINDOWS","TIMELINE","DNS","EDR"]},
  {id:"ransomware-corp",order:3,title:"Ransomware na Be Safe Corp",short:"SOC + DFIR + IR + crise executiva em um cenário encadeado.",track:"IR",level:"JR → PL",duration:"60–90 MIN",status:"planned",accent:"#ff5b5b",skills:["IR","DFIR","DECISÃO","CRISE"]},
  {id:"firewall-ha",order:4,title:"Firewall HA From Hell",short:"Rack, cabeamento, interfaces, VLANs, HA, NAT, políticas, VPN e failover.",track:"NETWORK",level:"JR → PL",duration:"60–120 MIN",status:"planned",accent:"#ff7a3d",skills:["NGFW","HA","VLAN","VPN"]},
  {id:"dfir-who",order:5,title:"DFIR — Quem fez isso?",short:"Artefatos, evidências e timeline para descobrir o vetor inicial.",track:"DFIR",level:"JR",duration:"45–60 MIN",status:"planned",accent:"#9e62ff",skills:["DFIR","FORENSICS","TIMELINE"]},
  {id:"hunt-no-alert",order:6,title:"Threat Hunting sem alerta",short:"Ninguém te deu uma regra pronta. Crie hipótese, procure comportamento e prove.",track:"HUNTING",level:"PL",duration:"45–60 MIN",status:"planned",accent:"#caff33",skills:["HUNTING","SIEM","HYPOTHESIS"]},
  {id:"mitre-detective",order:7,title:"MITRE ATT&CK Detective",short:"Converta comportamento bruto em técnicas, subtécnicas e cadeia de ataque.",track:"DETECTION",level:"START → JR",duration:"25–40 MIN",status:"planned",accent:"#5777ff",skills:["MITRE","DETECTION","ANALYSIS"]},
  {id:"phishing-center",order:8,title:"Phishing Investigation Center",short:"Headers, domínio, SPF/DKIM/DMARC, URLs, anexos e decisão de resposta.",track:"SOC",level:"START → JR",duration:"30–45 MIN",status:"planned",accent:"#ff3d98",skills:["EMAIL","IOC","SOC"]},
  {id:"web-attack",order:9,title:"SOC vs Web Attack",short:"Veja o mesmo ataque pelos olhos da aplicação e da defesa.",track:"APPSEC",level:"JR",duration:"45–60 MIN",status:"planned",accent:"#3de7ff",skills:["WAZUH","WEB","CORRELATION"]},
  {id:"cti-board",order:10,title:"CTI Murder Board",short:"Comece com um domínio e pivote até campanha, infraestrutura e ator.",track:"CTI",level:"JR → PL",duration:"45–75 MIN",status:"planned",accent:"#9e62ff",skills:["CTI","PIVOT","OPENCTI"]},
  {id:"ioc-intel",order:11,title:"IOC não é inteligência",short:"Transforme uma pilha de indicadores em inteligência acionável.",track:"CTI",level:"JR",duration:"30–45 MIN",status:"planned",accent:"#9e62ff",skills:["CTI","CONTEXT","CONFIDENCE"]},
  {id:"cloud-door",order:12,title:"Cloud — Quem deixou essa porta aberta?",short:"Permissões excessivas, secrets, storage público e caminhos de privilégio.",track:"CLOUD",level:"JR → PL",duration:"45–60 MIN",status:"planned",accent:"#5777ff",skills:["IAM","CLOUD","MISCONFIG"]},
  {id:"iam-disaster",order:13,title:"IAM Disaster",short:"Offboarding falhou. Encontre privilégios órfãos e quebre o caminho de abuso.",track:"IAM",level:"JR → PL",duration:"35–50 MIN",status:"planned",accent:"#caff33",skills:["JML","MFA","RBAC","IGA"]},
  {id:"soc-ai",order:14,title:"SOC + IA — Confie, mas nem tanto",short:"A IA analisa incidentes. Algumas respostas estão erradas. Valide tudo.",track:"AI",level:"JR → PL",duration:"30–45 MIN",status:"planned",accent:"#3de7ff",skills:["AI","VALIDATION","SOC"]},
  {id:"prompt-injection",order:15,title:"Prompt Injection Incident",short:"Defenda uma aplicação com LLM contra manipulação de instruções e abuso de contexto.",track:"AI",level:"PL",duration:"40–60 MIN",status:"planned",accent:"#ff3d98",skills:["LLM","GUARDRAILS","APPSEC"]},
  {id:"api-attack",order:16,title:"API Under Attack",short:"Autorização quebrada, abuso de autenticação, rate abuse e exposição de dados.",track:"APPSEC",level:"JR → PL",duration:"45–60 MIN",status:"planned",accent:"#ff7a3d",skills:["API","OWASP","AUTH"]},
  {id:"siem-cost",order:17,title:"O SIEM ficou caro pra cacete",short:"Escolha fontes, filtros, retenção, EPS e armazenamento sem matar visibilidade.",track:"ARCH",level:"PL",duration:"40–55 MIN",status:"planned",accent:"#caff33",skills:["SIEM","EPS","LOGS","COST"]},
  {id:"false-positive",order:18,title:"False Positive Factory",short:"Uma regra dispara milhares de vezes. Tune sem deixar o atacante passar.",track:"DETECTION",level:"JR → PL",duration:"30–45 MIN",status:"planned",accent:"#5777ff",skills:["TUNING","SIEM","FP"]},
  {id:"detection-engineering",order:19,title:"Detection Engineering",short:"Da técnica ofensiva à telemetria, regra, teste e redução de falsos positivos.",track:"DETECTION",level:"PL",duration:"45–70 MIN",status:"planned",accent:"#5777ff",skills:["SIGMA","MITRE","TEST"]},
  {id:"purple-arena",order:20,title:"Purple Team Arena",short:"Execute uma técnica controlada, valide cobertura e melhore a detecção.",track:"PURPLE",level:"PL",duration:"60–90 MIN",status:"planned",accent:"#9e62ff",skills:["PURPLE","BAS","DETECTION"]},
  {id:"linux-compromised",order:21,title:"Servidor Linux comprometido",short:"SSH, auth.log, cron, systemd, processos e conexões. Sem ferramenta mágica.",track:"DFIR",level:"JR",duration:"45–60 MIN",status:"planned",accent:"#caff33",skills:["LINUX","DFIR","SSH"]},
  {id:"windows-events",order:22,title:"Windows Event Logs Challenge",short:"Resolva o incidente quase só com Event Logs, Sysmon e PowerShell.",track:"DFIR",level:"START → JR",duration:"35–50 MIN",status:"planned",accent:"#3de7ff",skills:["WINDOWS","SYSMON","EVENTS"]},
  {id:"container-crime",order:23,title:"Container Crime Scene",short:"Imagem, secrets, processos e rede em um workload comprometido.",track:"CLOUD",level:"PL",duration:"45–60 MIN",status:"planned",accent:"#5777ff",skills:["CONTAINER","K8S","DFIR"]},
  {id:"ciso-30",order:24,title:"Você é o CISO por 30 minutos",short:"Orçamento curto, riscos demais e um board pedindo decisão.",track:"GRC",level:"PL → SR",duration:"30 MIN",status:"planned",accent:"#ff7a3d",skills:["RISK","BOARD","PRIORITY"]},
  {id:"cve-priority",order:25,title:"CVE ≠ prioridade",short:"500 vulnerabilidades. Descubra quais realmente merecem parar a fila.",track:"VM",level:"JR → PL",duration:"35–50 MIN",status:"planned",accent:"#caff33",skills:["CTEM","EPSS","KEV","RISK"]},
  {id:"incident-commander",order:26,title:"Incident Commander",short:"CEO, jurídico, DPO, SOC, imprensa e fornecedor perguntando ao mesmo tempo.",track:"IR",level:"PL → SR",duration:"40–60 MIN",status:"planned",accent:"#ff5b5b",skills:["CRISIS","IR","COMMS"]},
  {id:"timeline",order:27,title:"Timeline Challenge",short:"Firewall, AD, endpoint, proxy e DNS embaralhados. Reconstrua o ataque.",track:"DFIR",level:"JR",duration:"30–45 MIN",status:"planned",accent:"#3de7ff",skills:["TIMELINE","CORRELATION","SIEM"]},
  {id:"chain-custody",order:28,title:"Cadeia de Custódia Digital",short:"Colete, registre, hasheie e transfira evidências sem destruir a rastreabilidade.",track:"DFIR",level:"START → JR",duration:"30–45 MIN",status:"planned",accent:"#9e62ff",skills:["EVIDENCE","HASH","FORENSICS"]},
  {id:"security-architect",order:29,title:"Arquiteto de Segurança",short:"Requisitos, orçamento e riscos viram uma arquitetura que precisa fazer sentido.",track:"ARCH",level:"PL → SR",duration:"60–90 MIN",status:"planned",accent:"#ff7a3d",skills:["ARCH","DESIGN","TRADEOFF"]},
  {id:"presales",order:30,title:"Pré-vendas Cyber Challenge",short:"TR, P2P, sizing, BoM, arquitetura e pegadinhas de uma oportunidade realista.",track:"PRESALES",level:"JR → SR",duration:"60–120 MIN",status:"planned",accent:"#ff3d98",skills:["RFP","SIZING","BOM","P2P"]}
];
