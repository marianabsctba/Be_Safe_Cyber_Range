(() => {
  ...
  if (!profile?.name || !profile?.onboardingComplete) { location.href = `index.html?onboarding=1#workspace`; return; }

  if (!lab || lab.status !== 'live' || !lab.scenario) {
    root.innerHTML = `<main class="player-main"><div class="scenario-kicker">LAB INDISPONÍVEL</div><h1 class="scenario-title">Esse cenário ainda não entrou em produção.</h1><p class="scenario-copy">Volte ao Employee Portal e escolha uma missão disponível.</p><a class="button button-primary" href="index.html#labs">VOLTAR À FILA ↗</a></main>`;
    return;
  }

  const storageKey = `besafe-range:${lab.id}`;
  const freshAttempt = (carry={}) => ({
    stage:0, score:0, decisions:[], startedAt:Date.now(), updatedAt:Date.now(), finished:false,
    completedOnce:Boolean(carry.completedOnce), bestScore:Number.isFinite(carry.bestScore)?carry.bestScore:null,
    history:Array.isArray(carry.history)?carry.history:[], attempts:Math.max(1,Number(carry.attempts)||1)
  });
  let state = freshAttempt();
  try {
    const saved = JSON.parse(localStorage.getItem(storageKey));
    if (saved) {
      state = { ...freshAttempt(saved), ...saved, completedOnce:Boolean(saved.completedOnce || saved.finished), bestScore:Number.isFinite(saved.bestScore)?saved.bestScore:(Number.isFinite(saved.normalizedScore)?saved.normalizedScore:null), history:Array.isArray(saved.history)?saved.history:[] };
    }
  } catch (_) {}
  ...
  function persist() { state.updatedAt = Date.now(); localStorage.setItem(storageKey, JSON.stringify(state)); }
  ...
  function shell() {
    root.innerHTML = `
      <div class="lab-shell">
        <nav class="lab-nav">
          ...
          <div class="lab-nav-mid">BE SAFE CORP. INC. / ${lab.scenario.role}</div>
          <div class="lab-nav-right"><span class="timer" id="timer">00:00</span><a href="index.html#labs">SAIR ×</a></div>
        </nav>
        <div class="player-grid">
          <aside class="player-sidebar">
            <div class="side-label">WORKER // ${profile.employeeId}</div><div class="worker-in-lab">${profile.name}</div><div class="side-label">MISSION ${String(lab.order).padStart(2,'0')}</div>
            ...
            <button id="resetLab" class="button button-danger" style="width:100%;margin-top:12px">REINICIAR TENTATIVA</button>
          </aside>
    ...
    document.querySelector('#resetLab').addEventListener('click', () => {
      if (!confirm('Reiniciar esta tentativa? Seu melhor resultado e histórico de conclusão serão preservados.')) return;
      const carry = {completedOnce:state.completedOnce,bestScore:state.bestScore,history:state.history,attempts:(state.attempts||1)+1};
      state = freshAttempt(carry);
      persist();
      renderStage();
    });
    updateTimer();
  ...
  function renderStage() {
    ...
    main.innerHTML = `
      ...
      <div class="next-wrap ${previous?'show':''}" id="nextWrap"><button class="button button-lime" id="nextStage">${state.stage === stages.length-1 ? 'ENCERRAR MISSÃO / RELATÓRIO ↗' : 'PRÓXIMA ETAPA →'}</button></div>`;
  ...
  function grade(score) {
    ...
    if (score >= 80) return ['A','Missão concluída com excelente resposta'];
  ...
  function finish(restoring=false) {
    ...
    const secs = restoring && Number.isFinite(state.elapsedSeconds) ? state.elapsedSeconds : elapsed();
    const max = stages.reduce((sum,s)=> sum + Math.max(...s.options.map(o=>o.points)),0);
    const normalized = restoring && Number.isFinite(state.normalizedScore)
      ? state.normalizedScore
      : Math.max(0, Math.min(100, Math.round((state.score/max)*100)));
    ...
    if (!restoring) {
      ...
      state.normalizedScore = normalized;
      state.elapsedSeconds = secs;
      state.completedAt = Date.now();
      state.completedOnce = true;
      state.bestScore = Number.isFinite(state.bestScore) ? Math.max(state.bestScore, normalized) : normalized;
      state.history = Array.isArray(state.history) ? state.history : [];
      state.history.push({completedAt:state.completedAt,score:normalized,elapsedSeconds:secs,decisions:state.decisions.length});
      state.history = state.history.slice(-10);
      persist();
    }

    const summary = `BE SAFE CORP. INC. — RELATÓRIO INDIVIDUAL DE MISSÃO\n\nFuncionário(a): ${profile.name}\nEmployee ID: ${profile.employeeId}\nDepartamento: ${profile.department || 'SECURITY OPERATIONS'}\nEmpresa simulada: BE SAFE CORP. INC.\nMissão: ${lab.title}\nFunção: ${lab.scenario.role}\nResultado: ${letter} — ${label}\nScore desta tentativa: ${normalized}/100\nMelhor score: ${state.bestScore ?? normalized}/100\nTempo: ${fmt(secs)}\nTentativas registradas: ${state.history?.length || 1}\n\nDECISÕES\n${state.decisions.map((d,i)=>`${i+1}. ${d.label} (${d.points>=0?'+':''}${d.points})`).join('\n')}\n\nMITRE OBSERVADO\n${[...new Set(stages.flatMap(s=>s.mitre||[]))].join(', ')}\n\nLEITURA FINAL\nA missão exigia priorização por contexto, correlação entre fontes, contenção proporcional, ampliação de escopo e documentação objetiva. O score mede as decisões tomadas neste simulador e não substitui avaliação profissional real.`;

    document.querySelector('#stageMain').innerHTML = `
      <section class="report">
        <div class="report-head"><div class="scenario-kicker">${profile.name.toUpperCase()} / ${profile.employeeId} / MISSÃO CONCLUÍDA / ${fmt(secs)}</div><div class="report-grade">${letter}</div><h1>${label}</h1><p class="scenario-copy"><strong>${profile.name}</strong>, seu turno terminou. Seu melhor score fica preservado mesmo se você refizer esta missão.</p></div>
        <div class="report-grid">
          <div class="metric"><span>SCORE</span><strong>${normalized}/100</strong></div>
          <div class="metric"><span>MELHOR SCORE</span><strong>${state.bestScore ?? normalized}</strong></div>
        ...
        <div class="hero-actions report-actions"><button class="button button-primary" id="copyReport">COPIAR RELATÓRIO</button><button class="button button-ghost" id="printReport">SALVAR / IMPRIMIR PDF</button><button class="button button-ghost" id="retry">NOVA TENTATIVA</button><a class="button button-ghost" href="index.html#workspace">VOLTAR AO WORKSPACE</a></div>
      </section>`;
    ...
    document.querySelector('#retry').addEventListener('click', () => {
      const carry={completedOnce:true,bestScore:state.bestScore,history:state.history,attempts:(state.attempts||1)+1};
      state=freshAttempt(carry); persist(); location.reload();
    });
  }

  shell();
  if (state.finished) finish(true); else renderStage();
})();
