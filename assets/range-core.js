(function(){
  const KEY='bsc_range_v6_state';
  const deep=x=>JSON.parse(JSON.stringify(x));
  function initial(){
    return {
      version:6, employee:null, startedAt:null, virtualMinute:0, activeMission:'M01',
  ...
  function load(){ try{ const x=JSON.parse(localStorage.getItem(KEY)); return x&&x.version===6?x:initial(); }catch(e){return initial();} }
  let state=load();
  ...
  function startRansomware(){
    ...
    ransomwareLog('xdr','FILE-HR-01','ransomware_outbreak','BSC\\hrodrigues','10.20.33.19','10.40.30.25','BLACK FROST: encryption behavior confirmed; incident engine active');
  ...
  function advanceRansomware(minutes=0,action=''){
    ...
    if(r.elapsed>=8&&!r.networkSegmented){
      if(set('WS-HR-027','Encrypting',24)){r.impact.encryptedFiles+=24;r.timeline.push({time:clock(),type:'Lateral movement',detail:'WS-HR-027 began encryption after SMB activity from HR segment.'});ransomwareLog('ndr','NDR-SENSOR-01','ransom_smb_spread','svc-hr-sync','10.40.30.25','10.20.33.27','SMB fan-out followed by ransomware-like file operations');ransomwareNotify('Blast radius increased','WS-HR-027 is now encrypting. Network containment has not been applied.','ndr');if(state.xdr.endpoints['WS-HR-027'])state.xdr.endpoints['WS-HR-027'].risk='Critical';}
    }
  ...
  function audit(action,payload={}){ state.audit.push({ts:new Date().toISOString(),corpTime:clock(),action,payload}); const mins=Math.max(1,payload.minutes||2); state.virtualMinute+=mins; advanceRansomware(mins,action); save(); }
  ...
  window.BSC={get state(){return state},save,audit,gap,objective,notify,markApp,setEmployee,reset,clock,missionProgress,mutate,deep,startRansomware,advanceRansomware};
})();
