(function(){
  const apps=['workqueue','slack','teams','mail','itsm','ir','soc','xdr','firewall','waf','ndr','cmdb','topology','datacenter','vuln','iam','pam','dlp','cti','cloud','backup','dfir','detections','purple','grc','clients','appsec','attack','terminal','report'];
  const desktopApps=['workqueue','slack','teams','mail','itsm','ir','soc','xdr','firewall','waf','ndr','datacenter','pam','dfir','attack','topology','terminal','report'];
  const dockApps=['workqueue','slack','itsm','ir','soc','xdr','firewall','attack','terminal'];
  let z=30, drag=null;
  ...
  function renderIcons(){
    $('#desktopIcons').innerHTML=desktopApps.map(a=>{const [i,n]=BSCApps.meta[a];return `<button class="desktop-icon" data-app="${a}"><span class="icon">${i}</span><small>${n}</small></button>`}).join('');
    $('#dockApps').innerHTML=dockApps.map(a=>{const[i,n]=BSCApps.meta[a];return `<button class="dock-app" title="${n}" data-app="${a}">${i}</button>`}).join('');
    $$('[data-app]').forEach(b=>b.onclick=()=>openApp(b.dataset.app));
  }
  function renderMissionRail(){const rail=$('#missionRail');if(!rail)return;rail.innerHTML=`<div class="rail-head"><h3>YOUR MISSIONS</h3><button class="btn" id="closeMissionRail">×</button></div>${BSC_DATA.missions.map(m=>{const[d,t]=BSC.missionProgress(m.id);return `<article class="mission-card ${m.id===BSC.state.activeMission?'active':''}" data-set-mission="${m.id}"><header><strong>${m.id} · ${BSCApps.h(m.title)}</strong><small>${d}/${t}</small></header><p>${m.duration} · ${m.skills.join(' / ')}</p>${m.objectives.map(([k,v])=>`<div class="objective ${BSC.state.completedObjectives[k]?'done':''}"><span class="check">${BSC.state.completedObjectives[k]?'✓':''}</span><span>${BSCApps.h(v)}</span></div>`).join('')}</article>`}).join('')}`;$('#closeMissionRail').onclick=()=>rail.classList.add('hidden');rail.querySelectorAll('[data-set-mission]').forEach(x=>x.onclick=()=>{BSC.state.activeMission=x.dataset.setMission;BSC.audit('mission_selected',{mission:x.dataset.setMission,minutes:0});updateChrome()});}
  function renderNotifRail(){const rail=$('#notifRail');if(!rail)return;const ns=BSC.state.notifications;rail.innerHTML=`<div class="rail-head"><h3>NOTIFICATIONS</h3><button class="btn" id="closeNotifRail">×</button></div>${ns.length?ns.map(n=>`<div class="notification"><strong>${BSCApps.h(n.title)}</strong><p>${BSCApps.h(n.body)}</p><small>${BSCApps.h(n.time)} · ${BSCApps.h(n.app)}</small></div>`).join(''):'<div class="empty">No notifications yet.</div>'}`;$('#closeNotifRail').onclick=()=>rail.classList.add('hidden')}
  function renderStart(){const menu=$('#startMenu');if(!menu||!BSC.state.employee)return;const e=BSC.state.employee;menu.innerHTML=`<div class="rail-head"><h3>BE SAFE CORP .INC</h3><button class="btn" id="closeStart">×</button></div><div class="start-profile"><div class="avatar">${initials(e.name)}</div><div><b>${BSCApps.h(e.name)}</b><small>${BSCApps.h(e.email)}</small><small>${BSCApps.h(e.id)} · ${BSCApps.h(e.team)}</small></div></div><div class="start-apps">${apps.map(a=>{const[i,n]=BSCApps.meta[a];return `<button class="start-app" data-start-app="${a}"><span class="mini-icon">${i}</span><b>${n}</b></button>`}).join('')}</div><div class="toolbar" style="margin-top:12px"><button class="btn" id="resetRange">Reset local range</button></div>`;$('#closeStart').onclick=()=>menu.classList.add('hidden');menu.querySelectorAll('[data-start-app]').forEach(x=>x.onclick=()=>{menu.classList.add('hidden');openApp(x.dataset.startApp)});$('#resetRange').onclick=()=>{if(confirm('Resetar todo o progresso local deste Cyber Range?'))BSC.reset()}}
  ...
  function scheduleStory(){
    ...
    const events=[
      [1,'intro','SOCBOT','INC-2026-0001 assigned. 9 correlated signals are waiting for triage.','soc'],
      ...
      [32,'vm','Vulnerability Mgmt','APP-PROD-01 entrou na fila crítica. Contexto: internet-facing + KEV + EPSS alto.','vuln'],
      [42,'api','AppSec On-call','/api/login está 18x acima do baseline. Confirmem bot score antes de meter block global.','waf'],
      [52,'cloud','CSPM','bsc-marketing-export está public-read e houve GetObject anônimo.','cloud'],
      [62,'ransom','IR Lead','BLACK FROST confirmado: FILE-HR-01 está criptografando e há risco real de propagação lateral. Major Incident recomendado.','ir'],
      ...
      [82,'pam','PAM','adm_vendor abriu sessão privilegiada sem change correlato. Recording ativo.','pam'],
      [92,'ndr','NDR','East-west SMB fan-out saindo do jump host. Correlacionar identidade antes de bloquear a rede inteira.','ndr'],
      [108,'dc','DC Operations','Badge temporário de fornecedor entrou na row R03 sem escort registrado.','datacenter'],
      [122,'mss','BlueBank NOC','P1 do cliente: parceiro de compensação indisponível após bloqueio do SOC. SLA correndo.','clients'],
      [138,'purple','Security Manager','Janela de Purple Team liberada. Operation Glasshouse está autorizada dentro do range sintético.','attack']
    ];
  ...
  function init(){
    ...
    $('#onboardForm').addEventListener('submit',e=>{e.preventDefault();const name=$('#workerName').value.trim(),role=$('#workerRole').value,team=$('#workerTeam').value,id='BSC-'+String(Math.floor(10000+Math.random()*89999)),user=username(name),emp={name,role,team,id,username:user,email:`${user}@besafecorp.inc`,mfa:true,site:'BSC-HQ-SP',workstation:`WS-SOC-${String(Math.floor(10+Math.random()*89))}`};BSC.setEmployee(emp);$('#onboarding').classList.add('hidden');$('#desktop').classList.remove('hidden');afterLogin();toast(`Bem-vindo, ${name.split(' ')[0]}`,'Seu primeiro turno foi iniciado. Há trabalho esperando na fila.')});
  ...
  function afterLogin(){renderIcons();updateChrome();scheduleStory();$('#missionButton').onclick=()=>$('#missionRail').classList.toggle('hidden');$('#notifButton').onclick=()=>{BSC.state.notifications.forEach(n=>n.read=true);BSC.save();$('#notifRail').classList.toggle('hidden')};$('#startButton').onclick=()=>$('#startMenu').classList.toggle('hidden');$('#profileButton').onclick=()=>openApp('report');
    setInterval(()=>{$('#corpClock').textContent=BSC.clock()},15000);
    if(!BSC.state.audit.some(a=>a.action==='session_welcome')){BSC.audit('session_welcome',{minutes:0});BSC.notify('Paulo Martins / Security Manager','Bem-vindo. Comece pela Work Queue e pelo ticket atribuído. Documente o que fizer.','teams');}
  }
